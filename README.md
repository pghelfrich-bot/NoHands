...
- **Your images** — drag image files from your computer onto the drop zone above the search results (or click **browse**) to add them to the panel for this session; click or drag one onto a slide just like a search result. No attribution/license is attached, and the library itself isn't saved between sessions — only images you actually place on a slide are kept with the deck.
